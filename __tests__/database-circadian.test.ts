...
describe('Database - Circadian Patterns Table', () => {
  ...
  const createTestPattern = (
    overrides: Partial<Omit<CircadianPatternRecord, 'updated_at'>> = {}
  ...
  describe('Upsert circadian pattern', () => {
    it('should insert a new pattern when none exists for the hour', () => {
      ...
      const hourOfDay = upsertCircadianPattern(db, pattern);
    ...
    it('should update existing pattern when one exists for the hour', () => {
      ...
      const hourOfDay1 = upsertCircadianPattern(db, pattern1);

      const pattern2 = createTestPattern({
        hour_of_day: 10,
        avg_theta_mean: 2.0,
      });
      const hourOfDay2 = upsertCircadianPattern(db, pattern2);

      expect(hourOfDay2).toBe(hourOfDay1);
    ...
    it('should accept valid hours (0-23)', () => {
      [0, 12, 23].forEach((hour) => {
        ...
        const returnedHour = upsertCircadianPattern(db, pattern);
        expect(returnedHour).toBe(hour);
    ...
    it('should allow null avg_subjective_rating', () => {
      ...
      const retrieved = getCircadianPatternById(db, hourOfDay);
  ...
  describe('Get circadian pattern by ID (hour_of_day)', () => {
    it('should return null for non-existent hour', () => {
      const pattern = getCircadianPatternById(db, 999);
      expect(pattern).toBeNull();
    });

    it('should retrieve pattern by hour_of_day', () => {
      const patternData = createTestPattern();
      const hourOfDay = upsertCircadianPattern(db, patternData);

      const retrieved = getCircadianPatternById(db, hourOfDay);
      expect(retrieved).not.toBeNull();
      expect(retrieved?.hour_of_day).toBe(hourOfDay);
    });
  });
  ...
  describe('Update circadian pattern', () => {
    it('should update single field', () => {
      ...
      updateCircadianPattern(db, hourOfDay, { session_count: 10 });

      const updated = getCircadianPatternById(db, hourOfDay);
    ...
    it('should update multiple fields', () => {
      ...
      updateCircadianPattern(db, hourOfDay, {
        avg_theta_mean: 2.0,
        avg_theta_std: 0.5,
        session_count: 15,
      });
  ...
  describe('Delete circadian pattern', () => {
    it('should delete pattern by hour_of_day', () => {
      const pattern = createTestPattern();
      const hourOfDay = upsertCircadianPattern(db, pattern);

      deleteCircadianPattern(db, hourOfDay);
    ...
    it('should not affect other patterns when deleting one', () => {
      ...
      deleteCircadianPattern(db, hourOfDay1);

      const retrieved1 = getCircadianPatternById(db, hourOfDay1);
      const retrieved2 = getCircadianPatternById(db, hourOfDay2);
